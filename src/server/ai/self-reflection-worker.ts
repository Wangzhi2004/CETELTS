...
export async function runSelfReflectionPolicyUpdate() {
  ...
  try {
    ...
    // 3. Persist system updates. Avoid interactive transactions here because
    // Cloudflare's generated Prisma client can lose the callback parameter type.
    await prisma.oPEReport.create({
      data: {
    ...
    const usersToUpdate = (await prisma.userState.findMany({
